...
1. Locate the appropriate category under `backend/awa/tools/definitions/<category>.py`.
...
3. If Python translation is supported, implement the translator class under `backend/awa/translators/` and register it using `register_plugin` and `register_type`.
